...
const int     BEAT_INTERVAL_BUF_SIZE = 3;
unsigned long beatIntervalBuf[BEAT_INTERVAL_BUF_SIZE] = {};
...
void updateTempoByPhoto() {
  ...
  for(int i = BEAT_INTERVAL_BUF_SIZE-1; i >= 0; i--) {
    if (i == 0) beatIntervalBuf[i] = estimatedBeatInterval;
    else beatIntervalBuf[i] = beatIntervalBuf[i-1];
  };
};

// beatIntervalBuf[0]=今回, [1]=1つ前, [2]=2つ前
// 今回と2つ前が近い(=テンポは安定)のに1つ前だけ両隣と大きくずれているなら、
// 1つ前の記録は実際には拍を1回見逃して間隔が倍になっていたと判定する
void checkMissedBeat() {
  float now  = beatIntervalBuf[0];
  float prev = beatIntervalBuf[1];
  float far  = beatIntervalBuf[2];
  float diffNowFar  = abs(now - far)  / far * 100.0f;
  float diffPrevNow = abs(prev - now) / now * 100.0f;
  float diffPrevFar = abs(prev - far) / far * 100.0f;
  if (diffNowFar <= TEMPO_CHANGE_THRESH && diffPrevNow > TEMPO_CHANGE_THRESH && diffPrevFar > TEMPO_CHANGE_THRESH) {
...
void B_loop() {
  ...
  if (beatDetected) {
    ...
    if (state != IDLE) {
      ...
      updateTempoByPhoto();
      if (state == PLAYING) checkMissedBeat();
      beatUpdated = true;
    };
  };
  updateState();
