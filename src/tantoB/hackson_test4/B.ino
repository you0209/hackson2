...
// 動き検知（IDLE用、生値ベース）
float prevRawIrDistance = -1.0f; // -1 = 未初期化
const float RAW_MOVE_THRESH = 5.0f; // cm単位

// その他
const int INSTRUMENT_ID = 0;


// 生の距離値の変化量で腕の動きを検知（キャリブレーション前に使用）
bool detectRawMovement() {
  if (prevRawIrDistance < 0.0f) {
...
// VL53L0Xの値を取得
void readIr() {
  irDistance = sensor.readRangeContinuousMillimeters() / 10.0f;
...
// ===== テスト用: 1秒自動拍 =====
unsigned long lastAutoBeatTime = 0;
const unsigned long AUTO_BEAT_INTERVAL = 1000;

void autoDetectBeat() {
  unsigned long now = millis();
  if (now - lastAutoBeatTime < AUTO_BEAT_INTERVAL) return;
  lastAutoBeatTime = now;
  currentBeatTime = now;
  lastBeatTime = now;
  beatDetected = true;
  if (state == PREBEAT) prebeatCount++;
  if (state == PLAYING || state == TEMPO_CHANGE) isPlaying = true;
}
// ================================
