...
\begin{itemize}
  ...
  ADSRインスタンスを複数保持し，NOTE\_OFFフラグ=0の場合に空きスロットを割り当て，
  NOTE\_OFFフラグ=1の場合に該当スロットをReleaseへ移行させたうえで
  新規発音を行うことで多重発音（ポリフォニック）を実現する．
...
音量制御に反映する．
LONGバイトは音長（拍数）を示し，受信ログおよびデバッグ表示に活用する．

NOTE\_OFFフラグ=1のパケット受信時には前音の\texttt{adsr.noteOff()}を呼び出し，
続くNOTEバイトが休符（6）でなければ\texttt{adsr.noteOn()}で新規発音を開始する．
...
\begin{itemize}
  ...
  音名インデックス，Velocity，LONG（音長・拍数）を抽出して
