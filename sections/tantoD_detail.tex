...
\begin{figure}[H]  % 修正③：[h]→[H]（floatパッケージ使用）でTPM表の順序逆転を防止
  \centering
...
% 修正⑥：列幅不足で右端が切れていた問題を修正
%   - 表全体をsmallフォントで縮小
%   - フィルタ列・Bit列をp{}で幅指定し確実に収める
\begin{table}[h]
  \centering
...
\begin{figure}[H]  % 修正③：[h]→[H] でfloat位置を固定
  \centering
...
\begin{table}[H]  % 修正③：[h]→[H] で本文中の適切な位置に配置
  \centering
...
\begin{figure}[H]  % 修正③：[h]→[H] で本文との対応を固定
  \centering
...
% 修正③：TPM小見出しを表の直前に移動（PDF上で表→見出しの順序逆転していた問題を修正）
