...
5台のArduinoに対して，各楽器の演奏開始タイミング（オフセット）は表\ref{offset01}の通りである．なお，最終値は試聴によって調整を行う．
\begin{table}[H]
  \centering
  \caption{オフセット一覧表}
  \label{offset01}
...
1 & NOTE & 音名インデックス & uint8 &   0〜5（6 = 休符）\\ \hline
2 & VEL & 音の強さ & uint8 &   0〜127\\ \hline
3 & LONG & 音の長さ（拍数） & uint8 &   0〜127\\ \hline
  \end{tabular}  
\end{table}
\FloatBarrier
...
\begin{figure}[H]
    \centering
    \includegraphics[,width=0.8\linewidth, keepaspectratio]{figures/system.drawio.pdf}
    \caption{担当Cシステム構成図}
    \label{system}
\end{figure}
