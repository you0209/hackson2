...
LED制御は演奏フェーズによって2段階で切り替わる．演奏開始前の楽器指定フェーズではPWMデューティ比を変調して輝度レベルを指定し，各子機へ演奏開始キューを送信する．
演奏開始語はフル輝度（デューティ比100\%）のON/OFF制御に切り替え，拍信号として全子機へ送出する．\par

制御アプリケーション（Processing）は，楽器指定ボタン（楽器1~5），BPM入力欄，START・STOP・フェルマータボタンを備えた専用GUIインターフェースを提供する．
ユーザのボタン操作に応じてSTART・STOP・FERMATAの各コマンドをシリアル通信（115200bps）経由でArduinoへ送信する役割を担う．
加えて，BPM入力テキストボックスへの入力確定に応じて\texttt{BPM:xxx}コマンドを受信し，Arduinoはこれを受信してFspTimerの割り込み周期をリアルタイムに更新する．
...
\begin{tabular}{lll}
    ...
    \texttt{INSTRUMENT:n} & 楽器n版へキュー送信（輝度レベル変調） & 演奏中（キュー状態） \\
    \texttt{BPM:xxx} & BPM変更（\texttt{xxx}は60〜140の整数） & 状態維持（タイマ周期のみ更新） \\
    \hline
\end{tabular}
...
\begin{tabular}{llll}
    ...
    \texttt{setBPM(int bpm)} & 設定BPM値 \texttt{bpm} & なし & FspTimerの周期を再設定し，現在BPMを更新する \\
...
\begin{tabular}{llll}
    ...
    \texttt{drawButton(int x, int y, int w, int h, String label, color c)} & X座標，Y座標，高さ，幅，ラベル，色 & なし & 指定位置にボタンを描画する \\
    \texttt{validateAndSendBPM()} & 入力値を検証し（60〜140），BPMの数値を保存 \\
    \texttt{sendCommand(String cmd)} & 送信コマンド文字列 \texttt{cmd} & なし & シリアルポート経由でArduinoへコマンドを送信 \\
    \texttt{sendCue(int id)} & 楽器番号 \texttt{INSTRUMENT:n} & なし & シリアルポート経由でArduinoへ楽器番号を送信 \\
    \texttt{setBPM(int bpm)} & 演奏テンポ \texttt{BPM:bpm} & なし & シリアルポート経由でArduinoへBPMを送信 \\
    \hline
\end{tabular}
}
...
\begin{tabular}{llll}
    ...
    & BPM境界値検証 & 高速・低速動作時にサーボの追従遅れやタイマの処理落ちがないか & 最小設定BPM（60BPM）、最大設定BPM（140BPM） \\
    & BPMリアルタイム変更検証 & テキストボックスへの入力でBPMが即時反映され，サーボ動作が追従するか & 演奏中にBPMを変更し，LCDおよびサーボ周期を確認 \\
    & BPM範囲外入力検証 & 範囲外BPMが破棄され，LCDにエラー表示されるか & BPMテキストボックスへ59および141を入力 \\
    \hline
\end{tabular}
}
