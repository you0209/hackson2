...
uint8_t & note_off = 1 & 前の音の終了 \\ \hline
uint8_t & note = 0 & 音名インデックス（休符=6) \\ \hline
uint8_t & vel = 0 & 音の強さ（0~127) \\ \hline
uint8_t & note_long = 0 & 音の長さ（2,4,8) \\ \hline
...
\begin{table}[H]
 ...
 %   bool detectBeat() & なし（void）&bool（ true / false )& フォトトランジスタで親機LEDを検出しtrueを返す．そうでない場合はfalseを返す． \\ \hline
    unsigned long sendTime_set() & usingned long nextBeatTime& sendTime& 引数（次の拍の予測絶対時間）に補正値をかけて返す \\ \hline
