...
各楽器のオフセット一覧は表\ref{offset02}の通りである．なお，最終値は試聴によって調整を行う．
\begin{table}[H]
  \centering
  \caption{オフセット一覧表}
  \label{offset02}
...
\renewcommand{\lstlistingname}{ソースコード}
    ...
    const uint8_t BAR_OFFSET = 0;  //開始小節オフセット
...
\renewcommand{\lstlistingname}{ソースコード}
    ...
    uint16_t currentInterval = TEMPO_SHOW;  //現在の拍間隔[ms]
...
int & beatCount = 0 & 経過拍数（オフセット待機カウント） \\ \hline
int & remainBeats = 0 & 現在の音符の残り拍数 \\ \hline
bool & playing = false & 演奏開始フラグ（オフセット待機終了後に"true") \\ \hline
...
\begin{table}[H]
 ...
 %   bool detectBeat() & なし（void）&bool（ true / false )& フォトトランジスタで親機LEDを検出しtrueを返す．そうでない場合はfalseを返す． \\ \hline
    void onBeatDetected() & なし（void）& なし（void）& 拍検出時の処理：オフセット待機，テンポ切替，音符更新・送信，前の音の停止の送信を実行 \\ \hline
