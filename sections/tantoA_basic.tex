...
\begin{itemize}
    \item \textbf{テンポ提示機能}\\
    腕（反射板）を一定周期で滑らかに往復運動させ，１往復を１拍としてオーケストラ全体の店舗を提示する．
...
PCからの制御コマンド受信，FspTimerを用いた割り込み処理，およびマイクロ秒単位でのデバイス同期のフローを含む，システム全体の構成図を以下の図\ref{system}に示す．
...
ユーザインターフェースには，図\ref{user}に示すように「START」「STOP」「FERMATA」の各制御ボタンを配置する．
ユーザがPC上でこれらのボタンをクリックすることで，USB Type-Cを用いた石リアル通信を介して，対応する信号がArduinoへ送信される．

\begin{figure}[H]
    \centering
    \includegraphics[width=5cm]{figures/UI_image.png}
    \caption{ユーザーインターフェース表示案}
    \label{user}
\end{figure}

\subsection{状態遷移図}
本システムにおける状態遷移図を以下の図\ref{flowchart}に示す.
