...
\begin{itemize}
    \item \textbf{テンポ提示機能}\\
    腕（反射板）を一定周期で滑らかに往復運動させ，1往復を1拍としてオーケストラ全体の店舗を提示する．

    \item \textbf{拍の同期機能}\\
    腕が最前点に到達した瞬間に，反射板に搭載したLEDを短時間点灯させる．子機は「LEDの光」と「反射板までの距離の変動」の2つの物理的変化を観測することで，通信に頼らず正確に拍を同期する．

    \item \textbf{演奏状態の表現機能}\\
    「待機」「予備拍」「演奏中」「フェルマータ」「演奏停止」の各状態を持ち，状態に応じた動作の変化によって演奏の展開を表現する．

    \item \textbf{外部制御・情報表示機能}\\
    PC上の制御アプリケーションからシリアル通信経由で制御コマンド（START・STOP・FERMATA）を受信する．また．BPM入力テキストボックスを愛して演奏中を含む任意のタイミングでテンポをリアルタイムに変更できる．
    現在の演奏状態を本体のLCDディスプレイに表示し，ユニット単体での状況把握を可能にする．
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{figures/configuration.pdf}
    \caption{システム構成図}
    \label{tantoA:system}
\end{figure}
...
ユーザインターフェースには，図\ref{tantoA:user}に示すように「START」「STOP」「FERMATA」「楽器A〜E」の各制御ボタンを配置する．また，BPMを変更するための入力テキストボックスを配置し，現在のBPMを表示する．
ユーザがPC上でこれらのインターフェースを操作することで，USB Type-Cを用いたシリアル通信を介して，対応する信号がArduinoへ送信される．

\begin{figure}[H]
    \centering
    \includegraphics[width=5cm]{figures/UI_image.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{figures/conductor.pdf}
