...
\begin{description}
  ...
  \item[ソフトウェア]
    \begin{itemize}
      ...
      \item 演奏Arduino SW $\times$ 4（輪唱オフセット制御・楽譜進行・シリアル送信）
...
\begin{itemize}
  ...
  \item \textbf{演奏制御}：輪唱オフセットの管理，楽譜進行，テンポ追従演奏の3機能を担う．
...
\begin{itemize}
  \item \textbf{担当A（指揮者人形ユニット）}：サーボ機構・腕・反射板からなる人形機構，LED・サーボ駆動回路，およびサーボ制御・輝度エンコードLED制御ソフトウェア．
  \item \textbf{担当B（センサー・観測）}：フォトトランジスタ配線（遮光チャンバー含む），センサーISR・EMAテンポ推定・状態機械を実装した観測Arduino SW．
  \item \textbf{担当C（演奏制御）}：拍検出・オフセット管理・楽譜進行・シリアル送信を実装した演奏Arduino SW，および「かえるのうた」5パート分の楽譜配列データ．
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{lX}
    ...
    \midrule
    輪唱オフセット制御  & 演奏Arduino SW（拍検出・オフセット） \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{Xlcc}
    ...
    機材名 & 用途 & 数量 & 調達区分 \\
    \midrule
    Arduino Uno（またはNano） & 指揮者・観測・演奏各制御 & 6 & 既備 \\
    サーボモータ（SG90相当） & 指揮者人形の腕往復動作 & 1 & 既備 \\
    赤外線距離センサ（Sharp GP2Y0A21YK） & 腕位置連続観測 & 1 & 既備 \\
    フォトトランジスタ & 指揮者LED発光の拍検出 & 4 & 既備 \\
    高輝度LED（赤外線） & 指揮者側拍通知光源 & 2 & 既備 \\
    抵抗・コンデンサ類 & 各回路の定数設定 & 適量 & 既備 \\
    ブレッドボード・ジャンパ線 & プロトタイプ配線 & 適量 & 既備 \\
    スピーカ（パッシブ）& Processing音声出力 & 1 & 既備 \\
    工作材料（段ボール・スポンジ等） & 人形機構・遮光チャンバー筐体 & 適量 & 申請調達 \\
    PC（Processing実行用） & 音響合成・シリアル受信 & 1 & 持ち込み \\
    \bottomrule
  \end{tabularx}
\end{table}
