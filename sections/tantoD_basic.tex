...
\begin{itemize}
  ...
  音名インデックス，ベロシティ，および音長（拍数）情報を抽出する．
...
NOTE\_OFFフラグおよびNOTEバイトの受信パターンから推測可能な状態を
視覚的に表示するステータスパネルを実装する．

確定仕様の4バイトパケット（NOTE\_OFF / NOTE / VEL / LONG）には担当BのSTATE情報が
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    状態名 & 表示方法 & 説明 \\
    \hline
    IDLE          & 確実に判断可 & パケット未受信またはシリアル未接続 \\
    PLAYING       & 確実に判断可 & 発音パケット受信後，発音中 \\
    FERMATA       & 推測（目安2秒） & PLAYING中に次パケットが2秒以上未受信の場合 \\
...
なお，担当Cとの合意により4バイトパケットにSTATE情報（1バイト）を追加する場合は，
...
\begin{description}
  \item[IDLE]
  初期状態．シリアル入力待機中．
  発音パケット（NOTE $\neq$ 6）の受信によりPLAYING状態へ遷移する．

  \item[PLAYING]
  発音中の状態．ADSRに従って音を維持する．
  NOTE\_OFF=1を含む次パケットの受信によりReleaseフェーズを経てIDLEへ復帰する．
  フェルマータ中は担当Cが次パケットの送信を保留するため，
  Sustainが自動的に延長される．
