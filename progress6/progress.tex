...
216 & カラーLED制御SW実装 & 長見・飛田 & 50\% & カラーセンサ対応に変更 \\
...
216 & カラーLED制御SW実装 & 長見・飛田 & 50\% & \pLow & \pLow &  &  &  &  \\
...
226 & INSTRUMENT\_IDカラーキュー実装 & 長野・慶田 & 50\% & \pLow & \pLow &  &  &  &  \\
...
\midrule
241 & シリアル受信処理実装 & 中村・尾添 & 90\% & \pNone & \pNone &  &  &  &  \\
242 & 音色合成エンジン実装 & 中村・尾添 & 50\% & \pNone & \pNone &  &  &  &  \\
243 & エフェクト・フェルマータ延奏実装 & 中村・尾添 & 40\% & \pNone & \pNone &  &  &  &  \\
244 & 音色定義データ作成 & 中村・尾添 & 50\% & \pNone & \pNone &  &  &  &  \\
245 & ADSRパラメータ調整 & 中村・尾添 & 40\% & \pNone & \pNone &  &  &  &  \\
\midrule
...
313 & カラーLED・カラーセンサ検出確認 & 長見・飛田/長野・慶田 & — & \pNone & \pNone &  &  &  &  \\
...
\clearpage
\section{実績ガントチャート（第9週・6月24日時点）}

各メンバーの作業ログ（6月9日〜6月24日）に基づき，各週の実績進捗を色分けして示す．
進捗が確認できない週のセルは空白とする．
...
\begin{table}[H]
  ...
  \begin{tabular}{ll@{\hspace{1em}}ll@{\hspace{1em}}ll}
    \toprule
    \pLow & 80\%未満 & \pMid & 80\%以上100\%未満 & \pDone & 100\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{p{0.7cm}p{3.8cm}p{1.7cm}p{0.8cm}p{0.9cm}p{0.9cm}p{0.9cm}p{0.9cm}}
\toprule
ID & タスク名 & 担当者 & 進捗 & \makecell{5/27\\(水)} & \makecell{6/10\\(水)} & \makecell{6/17\\(水)} & \makecell{6/24\\(水)} \\
\midrule
\endfirsthead
...
\multicolumn{8}{l}{\textbf{【200 実装フェーズ ／ 210 担当A実装（指揮者人形ユニット）】}} \\
\midrule
211 & サーボ機構組み立て & 長見・飛田 & 0\% &  &  &  & \pLow \\
212 & 腕・反射板取り付け & 長見・飛田 & 0\% &  &  &  & \pLow \\
214 & サーボ駆動回路配線 & 長見・飛田 & 0\% &  &  &  &  \\
215 & サーボ制御SW実装 & 長見・飛田 & 80\% &  & \pLow & \pMid & \pMid \\
216 & カラーLED制御SW実装 & 長見・飛田 & 50\% &  &  & \pLow & \pLow \\
217 & Processing GUI実装 & 長見・飛田 & 100\% &  & \pDone &  &  \\
218 & シリアル通信実装（PC→Arduino） & 長見・飛田 & 100\% &  & \pDone &  &  \\
\midrule
\multicolumn{8}{l}{\textbf{【200 実装フェーズ ／ 220 担当B実装（センサー・観測）】}} \\
\midrule
221 & 赤外線センサー配線 & 長野・慶田 & 50\% & \pLow &  & \pLow & \pLow \\
222 & フォトTr配線・遮光チャンバー & 長野・慶田 & 70\% & \pLow &  & \pLow & \pLow \\
223 & センサーISR実装 & 長野・慶田 & 70\% & \pLow &  & \pLow & \pLow \\
224 & テンポ推定実装 & 長野・慶田 & 90\% & \pLow &  & \pLow & \pMid \\
226 & INSTRUMENT\_IDカラーキュー実装 & 長野・慶田 & 50\% &  &  &  & \pLow \\
227 & シリアル通信実装（担当C向け） & 長野・慶田 & 80\% &  &  & \pMid & \pMid \\
\midrule
\multicolumn{8}{l}{\textbf{【200 実装フェーズ ／ 230 担当C実装（演奏制御）】}} \\
\midrule
231 & 拍検出・オフセット実装 & 長山・慶田 & 80\% & \pMid & \pMid &  & \pMid \\
232 & 楽譜進行制御実装 & 長山・慶田 & 80\% & \pMid & \pMid &  & \pMid \\
\midrule
\multicolumn{8}{l}{\textbf{【200 実装フェーズ ／ 240 担当D実装（音響合成）】}} \\
\midrule
241 & シリアル受信処理実装 & 中村・尾添 & 90\% &  & \pLow & \pMid & \pMid \\
242 & 音色合成エンジン実装 & 中村・尾添 & 60\% &  & \pLow & \pLow & \pLow \\
243 & エフェクト・フェルマータ延奏実装 & 中村・尾添 & 40\% &  &  & \pLow & \pLow \\
244 & 音色定義データ作成 & 中村・尾添 & 50\% &  &  & \pLow & \pLow \\
245 & ADSRパラメータ調整 & 中村・尾添 & 40\% &  &  & \pLow & \pLow \\
\midrule
\multicolumn{8}{l}{\textbf{【300 テストフェーズ ／ 310 事前確認テスト】}} \\
\midrule
316 & Arduino-Processing通信確認 & 長野・長山・慶田/中村・尾添 & — &  &  & \pDone &  \\
\midrule
\multicolumn{8}{l}{\textbf{【300 テストフェーズ ／ 320 単体テスト】}} \\
\midrule
323 & 楽譜進行・シリアル送信単体テスト & 長山・慶田 & — &  &  & \pDone &  \\
\midrule
\multicolumn{8}{l}{\textbf{【300 テストフェーズ ／ 330 結合テスト】}} \\
\midrule
331 & Arduino-Processing遅延測定 & 長野・長山・慶田/中村・尾添 & — &  &  & \pDone &  \\
332 & 同期・テンポ追従確認 & 長野・長山・慶田 & — &  &  &  & \pLow \\
\midrule
\multicolumn{8}{l}{\textbf{【300 テストフェーズ ／ 340 システムテスト】}} \\
\midrule
341 & 通し演奏テスト & 全員 & — &  &  &  & \pLow \\
\midrule
